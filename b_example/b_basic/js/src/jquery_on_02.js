...
var enterOut = over.children('.out');
...
over.on ('mouseover', function() {
  ...
  sel.on('change', function(e){
   ...
   // var nar = $(this).val();
   // 선택된(select 요소 내부의 option)요소의 경우는 별도의 선택 되었을 때(selected)의 경우를 지정해야 한다.
  ...
  console.log(st);
  var pt = $('.position').offset().top; //-->scroll(0)기준
  console.log(pt);
  ...
  $(window).on('scroll', function(){
    ...
    $('body').css({backgroundColor:"#46abed"});
  });

  // offset 이해

  var porOffset = $('.por').offset().top;
  var porOffset = $('.pof').offset().top;
